...
async function getMGMTToken() {
    ...
    try {
        ...
        try {
            ...
            let token = await runner('az account get-access-token --resource=https://management.azure.com --query accessToken --output json')
            fs.writeFileSync( 'mgmt.json',JSON.stringify(token))
            return token
        } catch(error) {
